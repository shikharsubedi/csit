<div class="section">
	...
	<?php endif?> 
	<h4>Manage FAQs
    	<a href="<?php echo admin_url("faq/addfaqs/".$mid)?>" class="section-button" title="Add new media"><span class="ui-icon ui-icon-plusthick"></span><span class="icontext">Add New FAQs</span></a>
    </h4>                
    <div class="content">
  ...
  <?php if(count($faqss)): ?> 
    ...
    <table class="sortable data" cellpadding="0" cellspacing="0">
	  ...
	  <tbody id="toArray">
        <?php foreach ($faqss as $i):?> 
        <tr id="<?php echo $i["id"]?>">
			<td><input type="checkbox" name="check[]" value="<?php echo $i["id"]?>" class="flag-check"></td>
			
			<td><?php echo ucfirst($i["question"])?></td>
			<td><?php echo ($i["status"])?"Active":"Inactive"?></td>
            <td>
            	<div class="action-controls">
				<a href="<?php echo admin_url("faq/editfaqs"."/".$i["id"]) ?>" class="action-button" title="View and Edit Faq"><span class="ui-icon ui-icon-pencil"></span></a>
            <a href="<?php echo admin_url("faq/deletefaqs/".$i["id"]) ?>" class="action-button delete-image" title="Delete Faq"><span class="ui-icon ui-icon-trash"></span></a>
            <div class="clear"></div>
         </div>
            </td>
            </tr>
...
<table class="form-table">
    ...
    <?php else: ?>  
		No items available yet. Start by adding one <a href="<?php echo admin_url("faq/addfaqs/".$mid)?>" >here</a>
	<?php endif?>  
	<div style="clear"></div>
