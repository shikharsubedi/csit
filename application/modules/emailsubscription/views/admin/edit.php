       ...
       <form method="post" action="" name="frm_user" id="frm_user" enctype="multipart/form-data">
            <table width="500" border="0" class="form-table">
                <tr valign='top'>
                    ...
                    <td><input name="email" id="email" type="text"  value="<?php echo $email->getEmail() ?>"  class="regular-text required"/></td>
                </tr>
                <tr valign='top'>
                    <th scope="row"><label for="isActive">Is Active </label></th>
                    <td>
                        <input type="radio" name="status" id="isActive" value="<?php echo STATUS_ACTIVE ?>" checked=""/>
                        Yes&nbsp;&nbsp;
                        <input type="radio" name="status" id="isActive" value="<?php echo STATUS_INACTIVE ?>" <?php if($email->getActive()==STATUS_INACTIVE) echo 'checked';  ?>/>
                        No
                    </td>
                </tr>
