...
{
	$.get(base_url+'content/ajax/shortCodeTree',null,function(data,status,xhr){
...
CKEDITOR.on( 'dialogDefinition', function( ev )
	...
	types.commit = function(data) {
          ...
          if (data.type == 'ipage') {
			...
			//////////////////////////////////////////////
			var infoTab = dialogDefinition.getContents( 'code' );
		...
		infoTab.elements.push({
          ...
          children: [{
            type: 'select',
            id: 'f1_path',
            label: 'Select the shortcode',
            required: true,
			items: [["Loading Pages..",""]],
            onLoad: function() {
            	this.getInputElement().addClass('code-list');
              	$('.code-list').html(shortcode_data);
            ...
            validate: function() {
              ...
              if (dialog.getValueOf('code', 'linkType') != 'scode') {
                return true;
              }
              return true;
            }
	...
	types.items.push(['Short Code','scode']);
	...
	types.onChange = CKEDITOR.tools.override(types.onChange, function(original){
		...
		var element = dialog.getContentElement('code', 'drupalOptions').getElement().getParent().getParent();
		
		
		if (this.getValue() == 'scode') {
		  element.show();
	...
	types.commit = function(data) {
          ...
          if (data.type == 'scode') {
            ...
            dialog.setValueOf('code', 'protocol', '');
			
			var page_id = dialog.getValueOf('code', 'f1_path');
			var url_value = '[internal_link id='+page_id+']';
            dialog.setValueOf('code', 'url', url_value);
          }/**/
        };
